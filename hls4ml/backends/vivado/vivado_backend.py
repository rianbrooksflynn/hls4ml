...
class VivadoBackend(FPGABackend):
    ...
    def init_layernormalization(self, layer):
        if 'table_t' not in layer.attributes:
            layer.set_attr(
                'table_t', NamedType(name=layer.name + '_table_t', precision=FixedPrecisionType(width=21, integer=7))
            )
        if 'table_size' not in layer.attributes:
            layer.set_attr('table_size', 256)  # table size
        if 'table_range' not in layer.attributes:
            layer.set_attr('table_range', 1.0)  # table range
        if 'mean_t' not in layer.attributes:
            layer.set_attr(
                'mean_t', NamedType(name=layer.name + '_mean_t', precision=FixedPrecisionType(width=22, integer=6))
            )
