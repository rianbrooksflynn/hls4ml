...
dense_config_template = """struct config{index}_dense_{config_label} : nnet::dense_config {{
...
hept_config_template = """struct config{index} : nnet::hept_config {{
    ...
    typedef {config_dense_alpha} dense_alpha_conf;
    typedef {config_dense_qkv} dense_qkv_conf;

    static const unsigned n_hashes = {n_hashes};
    static const unsigned n_heads = {n_heads};
    static const unsigned padded_size = {padded_size};
...
hept_function_template = """nnet::hept<{input_t}, {output_t}, {config}>({input_q}, {input_k}, {input_v}, {input_shifts}, {output});"""
...
class HeptConfigTemplate(LayerConfigTemplate):
    ...
    def format(self, node):
        ...
        params['n_hashes'] = node.get_attr('n_hashes')
        params['n_heads'] = node.get_attr('n_heads')
        params['padded_size'] = node.get_attr('padded_size')
        ...
        params['config_dense_alpha'] = f'config{node.index}_dense_alpha'
        ...
        # dense alpha config
        dense_alpha_params = self._default_config_params(node)
        dense_alpha_params['config_label'] = 'alpha'
        dense_alpha_params['strategy'] = 'latency'
        dense_alpha_params['n_in'] = params['dim_per_head'] + params['coords_dim']
        dense_alpha_params['n_out'] = params['n_hashes']
        dense_alpha_params['product_type'] = get_backend('vivado').product_type(
            node.get_input_variable().type.precision, node.get_input_variable().type.precision
        )
        dense_alpha_params['weight_type'] = node.get_input_variable().type.name
        dense_alpha_params['bias_type'] = node.get_input_variable().type.name
        dense_alpha_params['reuse'] = params['reuse']
        dense_alpha_params['index'] = str(node.index)
        dense_alpha_params['nzeros'] = 0
        dense_alpha_params['nonzeros'] = (params['dim_per_head'] + params['coords_dim']) * params['n_hashes']
        dense_alpha_params['dense_function'] = 'DenseLatency'
        dense_alpha_config = self.dense_conf_qkv_template.format(**dense_alpha_params)

        # dense qkv config
        dense_qkv_params = self._default_config_params(node)
        dense_qkv_params['config_label'] = 'qkv'
...
class HeptFunctionTemplate(FunctionCallTemplate):
    ...
    def format(self, node):
        ...
        params['input_shifts'] = node.model.get_layer_output_variable(node.inputs[3]).name
